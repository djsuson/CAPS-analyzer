...
analyzer_source_hdl analyzer_source_from_file(const char *file_name, analyzer_data_t which) {
  unsigned n = 0, m = 0;
  ...
  if (strstr(file_name,".fits"))
    s_filesource[n] = new(std::nothrow) fitsManager(file_name);
  else if (strstr(file_name,".csv"))
    s_filesource[n] = new(std::nothrow) csvManager(file_name);
...
analyzer_source_hdl analyzer_source_header_from_file(const char* file_name, analyzer_data_t which) {
  unsigned n = 0;
...
int analyzer_use_slices(analyzer_source_hdl sh, int min_slice, int max_slice) {
  fileManager *fm = 0;
  ...
  if (min_slice < 0 || max_slice - min_slice > fm->slices())
    return ANALYZER_FUNCTION_PARAM_ERR;

  fm->min_slice(min_slice);
  fm->max_slice(max_slice);
...
int analyzer_sky_new_empty(analyzer_hdl ah, analyzer_data_t which, double ra_res, double dec_res) {
  ...
  collection->addEmpty(FILETYPE(which),x,y);
...
int analyzer_ps_transfer_indices(analyzer_power_spect_hdl ph, analyzer_hdl ah)
{
 ...
 spect->maxIndex(collection->transformedData()->maxIndex());
 spect->minIndex(collection->transformedData()->minIndex());
...
int analyzer_data_to_file(analyzer_hdl ah, analyzer_data_t d_type, analyzer_file_t f_type, const char *file_name) {
  ...
  switch (f_type) {
    case ANALYZER_FILE_FITS:
      fm = new(std::nothrow) fitsManager(data,file_name);
      if (!fm)
        return ANALYZER_ALLOC_ERR;
    ...
    case ANALYZER_FILE_CSV:
      fm = new(std::nothrow) csvManager(data,file_name);
      if (!fm)
        return ANALYZER_ALLOC_ERR;
